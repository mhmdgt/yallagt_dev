...
use App\Models\EngineCc as table;
...
class EngineCapacity extends Component
{
    ...
    {
        $ccTypes = table::latest()->paginate(10, ['name', 'logo', 'id']);
        return view('livewire.gt-manager.cars-assets.spec-categories.engine-capacity', compact('ccTypes'));
    }
    ...
    {
        $ccTypes = table::findOrFail($id);
        $this->logo = $ccTypes ? $ccTypes->logo : null;
        $this->name_en = $ccTypes ? $ccTypes->getTranslations('name')['en'] : '';
        $this->name_ar = $ccTypes ? $ccTypes->getTranslations('name')['ar'] : '';
        $this->clearValidationErrors();
    }
    // -------------------- update -------------------- //
    public function update($id)
    {
        ...
        $ccTypes->update([
            "name" => ['en' => $validatedData['name_en'] ?? $ccTypes->name->en, 'ar' => $validatedData['name_ar'] ?? $ccTypes->name->ar],
            "slug" => $this->slug(['en' => $validatedData['name_en'] ?? $ccTypes->name->en, 'ar' => $validatedData['name_ar'] ?? $ccTypes->name->ar]),
            "logo" => $this->logo instanceof \Illuminate\Http\UploadedFile  ?
            $this->logo->store('media/spec_category_imgs/engine_cc', 'public') :
            $ccTypes->logo,
        ]);
