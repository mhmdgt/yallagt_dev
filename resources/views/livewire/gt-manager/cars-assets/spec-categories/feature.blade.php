                                            ...
                                            <td>
                                                <div>
                                                    <img class="image-in-circle-75"
                                                        src="{{ !empty($feature->logo) ? asset('storage/' . $feature->logo) : asset('gt_manager/media/no_image.jpg') }}"
                                                        alt="profile">
                                                </div>
                                            </td>
                                            <td>{{ $feature->getTranslations('name')['en'] }}</td>
                                            <td> {{ $feature->getTranslations('name')['ar'] }}</td>
                                            <td class="d-flex">
                                                <button class="btn btn-inverse-warning mr-2"
                                                    wire:click="edit({{ $feature->id }})" data-toggle="modal"
                                                    data-target="#editModal{{ $feature->id }}" title="Edit">
                                                    Edit
                                                </button>
                                                <x-modal.delete id="{{ $feature->id }}">
                                                    <button type="button" class="btn btn-inverse-danger" data-toggle="modal"
                                                    data-target="#confirmDeleteModal{{ $feature->id}}">Delete</button>
                                                </x-modal.delete>
                                            </td>
                                    </tr>
                                    {{-- ========================== Edit Modal ========================== --}}
                                    <x-modal.edit title="Color" id="{{ $feature->id }}">
